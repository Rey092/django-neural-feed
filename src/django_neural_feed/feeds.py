...
class BaseNeuralFeed:
    ...
    @classmethod
    def _get_model(cls):
        if cls.model_name not in cls._model_instances:
            from sentence_transformers import SentenceTransformer

            try:
                cls._model_instances[cls.model_name] = SentenceTransformer(
                    cls.model_name, local_files_only=True
                )
            except Exception:
                cls._model_instances[cls.model_name] = SentenceTransformer(
                    cls.model_name, local_files_only=False
                )

        return cls._model_instances[cls.model_name]
    ...
    def get_user_vector(cls, user) -> list[float] | None:
        return getattr(user, "user_embedding", None)

    @classmethod
    def get_candidates(cls, user, queryset, excluded_ids=None):
        if queryset is None:
            queryset = cls.content_model.objects.all()  # type: ignore
