...
def test_calculate_user_embedding_with_real_db():
    ...
    TestUserAction.objects.create(user=user, embedding=[1.0, 2.0, 3.0], action_type="like")
    TestUserAction.objects.create(user=user, embedding=[2.0, 4.0, 6.0], action_type="like")
    TestUserAction.objects.create(user=user, embedding=[3.0, 6.0, 9.0], action_type="like")
...
def test_get_feed_for_user_sorting_and_filtering(mocker):
    ...
    mocker.patch.object(RecommendationService, 'calculate_user_embedding', return_value=[1.0, 0.0, 0.0])
    ...
    assert feed.count() == 2
    assert feed[0].id == post_closest.id  # Must be first due to highest vector proximity # type: ignore
